...
namespace XLua.CSObjectWrap
{
    ...
    public class cfgitemTbItemExtraWrap 
    {
        public static void __Register(RealStatePtr L)
        {
			ObjectTranslator translator = ObjectTranslatorPool.Instance.Find(L);
			System.Type type = typeof(cfg.item.TbItemExtra);
        ...
        static int __CreateInstance(RealStatePtr L)
        {
			...
			try {
				...
				if(LuaAPI.lua_gettop(L) == 2 && translator.Assignable<Bright.Serialization.ByteBuf>(L, 2))
				{
					...
					var gen_ret = new cfg.item.TbItemExtra(__buf);
            ...
            return LuaAPI.luaL_error(L, "invalid arguments to cfg.item.TbItemExtra constructor!");
        ...
        public static int __CSIndexer(RealStatePtr L)
        {
			try {
				...
				if (translator.Assignable<cfg.item.TbItemExtra>(L, 1) && LuaTypes.LUA_TNUMBER == LuaAPI.lua_type(L, 2))
				{
					
					cfg.item.TbItemExtra gen_to_be_invoked = (cfg.item.TbItemExtra)translator.FastGetCSObj(L, 1);
